...
public class ProgramNavModule {
    ...
    public boolean validateDashboardPage() {
        if (driver.getTitle().equals("LMS") && lmsHeading.isDisplayed()) {
            ...
            for (WebElement headers : headers) {
                String expectedText = headers.getText();
                if (headers.getText().equals(expectedText)) {
                    Assert.assertTrue(true, "Heading validated: " + headers.getText());
                } else {
                    Assert.fail("Unexpected heading text: " + headers.getText());
                    return false;
                }
            }
    ...
    public String lmsHeading() {
        return Utility_Methods.getElementText(lmsHeading);
